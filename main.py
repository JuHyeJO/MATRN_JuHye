...
def _get_language_databaunch(config):
    ...
    print(f'{len(data.train_ds)} training items found.')
...
def _get_databaunch(config):
    ...
    if not data.empty_val:
        ...
        print(f'{len(data.valid_ds)} valid items found.')
        print(f'{len(data.test_ds)} test items found.')
    return data
...
def _get_model(config):
    ...
    if config.global_phase == 'train':
        print(model)
        # logging.info(model)
    # logging.info(f'num_params: {sum(p.numel() for p in model.parameters() if p.requires_grad)}')
...
def main():
    ...
    parser.add_argument('--test_bh', type=str, default=None)

    parser.add_argument("--iter_size", type=int, default=None)
    ...
    if args.test_bh is not None: config.test_bh = args.test_bh

    if args.iter_size is not None: config.model_iter_size = args.iter_size
    if args.test_batch_size is not None: config.dataset_test_batch_size = args.test_batch_size

    config.global_workdir = os.path.join(config.global_workdir, config.global_name)
    save_root = '/gdrive/MyDrive/AI/MATRN'
    dataset_root = '/gdrive/MyDrive/AI/data_lmdb'

    config.global_workdir = os.path.join(save_root, config.global_workdir)
    config.dataset_train_roots = [os.path.join(dataset_root, root) for root in config.dataset_train_roots]
    config.dataset_test_roots = [os.path.join(dataset_root, root) for root in config.dataset_test_roots]
    config.dataset_valid_roots = [os.path.join(dataset_root, root) for root in config.dataset_valid_roots]

    if config.model_checkpoint is not None:
        config.model_checkpoint = f"{config.global_workdir}/{config.model_checkpoint}"
    if config.model_vision_checkpoint is not None:
        config.model_vision_checkpoint = f"{save_root}/{config.model_vision_checkpoint}"
    if config.model_language_checkpoint is not None:
        config.model_language_checkpoint = f"{save_root}/{config.model_language_checkpoint}"
    if config.model_alignment_checkpoint is not None:
        config.model_alignment_checkpoint = f"{save_root}/{config.model_alignment_checkpoint}"
    ...
    if args.local_rank is not None:
        ...
        print(f'Init distribution training at device {args.local_rank}.')
    ...
    if config.global_phase == 'train':
        ...
        logging.info(f'validation time: {valid_time} / batch size: {config.dataset_test_batch_size}')
